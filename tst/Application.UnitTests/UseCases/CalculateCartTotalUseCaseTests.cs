...
public class CalculateCartTotalUseCaseTests
{
    ...
    public async Task CalculateTotalAsync_ValidInput_CalculatesCartTotal()
    {
        ...
        var product1 = new Product("Product1", 10.0m, 5);
        var product2 = new Product("Product2", 15.0m, 3);
        ...
        // Act
        var total = await useCase.CalculateTotalAsync(input);

        // Assert
        var expectedTotal = (2 * 10.0m + 1 * 15.0m) * (1 + 0.08M); // Subtotal + Tax
