...
public class AddItemToCartUseCaseTests
{
    ...
    public async Task AddItemToCartAsync_ShouldCreateNewCart_WhenCartDoesNotExist()
    {
        ...
        // Assert
        await shoppingCartRepository.Received(1).SaveAsync(Arg.Is<ShoppingCart>(cart => cart.UserId == userId &&
    ...
    public async Task AddItemToCartAsync_ShouldThrow_WhenRequestedQuantityExceedsStock()
    {
        ...
        var product = new Product("Limited Edition Item", 25.00m, 1); // Only 1 in stock
        ...
        var input = new AddItemToCartInput(userId, product.Id, 2); // Requesting more than available
