...
public class PaymentGateway(IPaymentGatewayApi paymentGatewayApi) : IPaymentGateway
{
    private readonly IPaymentGatewayApi _paymentGatewayApi =
        paymentGatewayApi ?? throw new ArgumentNullException(nameof(paymentGatewayApi));

    public async Task<PaymentResult> ProcessPaymentAsync(PaymentRequest paymentRequest)
    {
        var result = await _paymentGatewayApi.ProcessPaymentAsync(paymentRequest);
    ...
    {
        var result = await _paymentGatewayApi.GetPaymentStatusAsync(paymentId);
